...
class Decensor:
    ...
    def load_model(self):
        self.model = InpaintNN(bar_model_name="./models/bar/Train_775000.meta", bar_checkpoint_name="./models/bar/",
                               mosaic_model_name="./models/mosaic/Train_290000.meta", mosaic_checkpoint_name="./models/mosaic/", is_mosaic=self.is_mosaic)
    ...
    def decensor_all_images_in_folder(self):
        ...
        for file_name in file_names:
            ...
            if os.path.isfile(color_file_path) and color_ext.casefold() == ".png":
                log("--------------------------------------------------------------------------", Ansi.GRAY)
                log("Decensoring the image {}".format(
                    color_file_path), Ansi.CYAN)
                try:
                    colored_img = Image.open(color_file_path)
                except:
                    log("Cannot identify image file (" +
                        str(color_file_path)+")", Ansi.RED)
                ...
                if self.is_mosaic:
                    ...
                    for test_file_name in os.listdir(ori_dir):
                        ...
                        log("Corresponding original, uncolored image not found in {}".format(
                            color_file_path), Ansi.RED)
    ...
    def decensor_image(self, ori, colored, file_name=None):
        ...
        regions = find_regions(colored.convert(
            'RGB'), [v*255 for v in self.mask_color])
        log("Found {region_count} censored regions in this image!".format(
            region_count=len(regions)), Ansi.CYAN)
        if len(regions) == 0 and not self.is_mosaic:
            log("No green regions detected! Make sure you're using exactly the right color", Ansi.RED)
            return
        ...
        for region_counter, region in enumerate(regions, 1):
            ...
            pred_img_array = (
                255.0 * ((pred_img_array + 1.0) / 2.0)).astype(np.uint8)
            ...
            log("{region_counter} out of {region_count} regions decensored.".format(
                region_counter=region_counter, region_count=len(regions)), Ansi.GRAY)
        ...
        if file_name != None:
            ...
            log("Decensored image saved to {save_path}!".format(
                save_path=save_path), Ansi.GREEN)
            return
        else:
            log("Decensored image. Returning it.", Ansi.GREEN)
            return output_img
