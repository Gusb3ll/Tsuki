...
def decoder(input, size1, size2, reuse, name):
    with tf.compat.v1.variable_scope(name):
        ...
        LL2 = layers.conv2d(DL4, 3, [3, 3], stride=[
                            1, 1], padding='SAME', activation_fn=None)  # 256 256 3
...
def contextual_block(bg_in, fg_in, mask, k_size, lamda, name, stride=1):
    with tf.compat.v1.variable_scope(name):
        ...
        patch1 = tf.compat.v1.extract_image_patches(
            bg, [1, k_size, k_size, 1], [1, stride, stride, 1], [1, 1, 1, 1], 'VALID')
        ...
        patch2 = tf.compat.v1.extract_image_patches(fg_in, [1, k_size, k_size, 1], [
        ...
        for ib in range(b):
            ...
            DS2 = (DS1 - tf.reduce_mean(DS1, 3, True)) / \
                reduce_std(DS1, 3, True)
...
def contextual_block_cs(bg_in, fg_in, mask, k_size, lamda, name, stride=1):
    with tf.variable_scope(name):
        ...
        for ib in range(b):
            ...
            CA = tf.reshape(
                CA, [1, h, w, int(math.sqrt(c)), int(math.sqrt(c))])
