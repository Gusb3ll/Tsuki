...
def ff_mask_batch(size, b_size, maxLen, maxWid, maxAng, maxNum, maxVer, minLen=20, minWid=15, minVer=5):
    ...
    mask = None
    temp = ff_mask(size, 1, maxLen, maxWid, maxAng, maxNum,
                   maxVer, minLen=minLen, minWid=minWid, minVer=minVer)
    temp = temp[0]
...
def spectral_norm(w, name, iteration=1):
    ...
    u = tf.compat.v1.get_variable(
        name+"u", [1, w_shape[-1]], initializer=tf.compat.v1.truncated_normal_initializer(), trainable=False)
...
def convolution_SN(tensor, output_dim, kernel_size, stride, name):
    ...
    w = tf.compat.v1.get_variable(
        name=name + 'w', shape=[kernel_size, kernel_size, c, output_dim], initializer=layers.xavier_initializer())
    b = tf.compat.v1.get_variable(
        name=name + 'b', shape=[output_dim], initializer=tf.constant_initializer(0.0))

    output = tf.nn.conv2d(tensor, filters=spectral_norm(
        w, name=name + 'w'), strides=[1, stride, stride, 1], padding='SAME') + b
...
def dense_SN(tensor, output_dim, name):
    ...
    w = tf.get_variable(
        name=name + 'w', shape=[h, w, c, output_dim], initializer=layers.xavier_initializer())
    b = tf.get_variable(
        name=name + 'b', shape=[output_dim], initializer=tf.constant_initializer(0.0))

    output = tf.nn.conv2d(tensor, filter=spectral_norm(
        w, name=name + 'w'), strides=[1, 1, 1, 1], padding='VALID') + b
...
def dense_RED_SN(tensor, name):
    ...
    weight = tf.compat.v1.get_variable(
        name=name + '_w', shape=[h*w, 1, c, 1], initializer=layers.xavier_initializer())
    b = tf.compat.v1.get_variable(
        name=name + '_b', shape=[1, h, w, 1], initializer=tf.constant_initializer(0.0))
